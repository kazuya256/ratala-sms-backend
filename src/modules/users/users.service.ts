...
export class UsersService {
    ...
    async findOneByUsername(username: string): Promise<User | null> {
        return this.userRepository.findOne({
            ...
            select: ['id', 'username', 'password', 'role', 'isActive', 'isVerified', 'refreshToken', 'refreshExpires', 'loginAttempts', 'lockUntil', 'tokenVersion'] as any,
            relations: ['studentProfile', 'studentProfile.class', 'studentProfile.section'],
        });
    }
    ...
    async incrementLoginAttempts(id: string): Promise<void> {
        ...
        if (user) {
            ...
            const lockUntil = attempts >= 5 ? new Date(Date.now() + 60 * 60 * 1000) : null; // 1 hour lock
            await this.userRepository.update(id, { loginAttempts: attempts, lockUntil });
        }
    }
    ...
    async findUserWithRefreshToken(id: string): Promise<User | null> {
        return this.userRepository.findOne({
            where: { id } as any,
            select: ['id', 'username', 'role', 'refreshToken', 'refreshExpires'] as any,
        });
    }
    ...
    async getMyChildren(parentId: string): Promise<any[]> {
        const parent = await this.userRepository.findOne({
            ...
            relations: ['children', 'children.studentProfile', 'children.studentProfile.class', 'children.studentProfile.section'],
        });
    ...
    async searchUsers(query: string, role: UserRole) {
        const relations: string[] = [];
        if (role === UserRole.STUDENT) {
            relations.push('studentProfile', 'studentProfile.class', 'studentProfile.section');
        }

        const users = await this.userRepository.find({
            where: [
                { username: Like(`%${query}%`), role },
            ],
            relations,
            take: 20
        });
    ...
    async updateStudentProfile(id: string, data: Partial<Student>) {
        const student = await this.studentRepository.findOne({ where: { user: { id } } });
    ...
    async updateTeacherProfile(id: string, data: Partial<Teacher>) {
        const teacher = await this.userRepository.manager.getRepository('Teacher').findOne({ where: { user: { id } } });
    ...
    async updateParentProfile(id: string, data: Partial<Parent>) {
        const parent = await this.userRepository.manager.getRepository('Parent').findOne({ where: { user: { id } } });
    ...
    async ensureTeacherProfile(userId: string): Promise<Teacher> {
        let teacher = await this.userRepository.manager.getRepository(Teacher).findOne({
        ...
        if (!teacher) {
            ...
            teacher = this.userRepository.manager.getRepository(Teacher).create({ user: { id: user.id } as any });
