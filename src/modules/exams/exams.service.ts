...
export class ExamsService {
  ...
  async createExam(data: any): Promise<Exam> {
    ...
    if (data.class?.id)
      data.class = (await this.classRepository.findOne({
        where: { id: data.class.id } as any,
      })) || { id: data.class.id };
    if (data.section?.id)
      data.section = (await this.sectionRepository.findOne({
        where: { id: data.section.id } as any,
      })) || { id: data.section.id };
    if (data.subject?.id)
      data.subject = (await this.subjectRepository.findOne({
        where: { id: data.subject.id } as any,
      })) || { id: data.subject.id };
  ...
  async bulkUploadMarks(examId: string, marksData: any[]) {
    ...
    for (const data of marksData) {
      ...
      if (!studentId && data.rollNumber) {
        const student = await this.sectionRepository.manager
      ...
      if (!studentId && data.username) {
        const user = await this.sectionRepository.manager
          .getRepository('User')
          .findOne({
