...
export class ReportsService {
  ...
  async getStudentAcademicReport(studentId: string) {
    const student =
      await this.usersService.findOneStudentWithProfile(studentId);
  ...
  async getAdminDashboardStats() {
    ...
    const recentPayments = (await this.feesService.getRecentPayments()).map(
      (p) => ({
        id: p.id,
        amount: p.amount,
    ...
    const attendanceTrends =
      await this.attendanceService.getDailyAttendanceSummaryInRange(
        sevenDaysAgo,
        new Date(),
      );
    ...
    const revenueTrends = await this.feesService.getRevenueInRange(
      sixMonthsAgo,
      new Date(),
    );
  ...
  async getAttendanceSummaryByClass(date?: string) {
    ...
    const allAttendance = await this.attendanceService.getClassAttendance(
      undefined,
      undefined,
      today,
    );
    ...
    const summaries: Record<
      string,
      { classSec: string; present: number; absent: number; total: number }
    > = {};
