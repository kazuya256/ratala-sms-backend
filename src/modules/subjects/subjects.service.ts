...
export class SubjectsService {
    ...
    // Granular Allocation
    async allocate(data: { teacherId: string, subjectId: string, sectionId: string }) {
        console.log('Allocating subject:', data);

        const teacher = await this.usersService.ensureTeacherProfile(data.teacherId);
        const subject = await this.subjectRepository.findOne({ where: { id: data.subjectId } as any });
        const section = await this.sectionRepository.findOne({ where: { id: data.sectionId } as any, relations: ['class'] });

        if (!subject || !section) throw new NotFoundException('Subject or Section not found');
    ...
    async findAllAllocations() {
        return this.allocationRepository.find({
            relations: ['teacher', 'teacher.user', 'subject', 'section', 'section.class']
        });
    }
