...
export class TimetableService {
  ...
  async getAllByClass(classId: string): Promise<Timetable[]> {
    ...
    console.log(
      `[Timetable] Found ${results.length} total entries for class ${classId}`,
    );
    return results;
  }

  /** Filter by class + section. If no valid sectionId, returns ALL entries for the class. */
  async getTimetableByClassAndSection(
    classId: string,
    sectionId: string,
  ): Promise<Timetable[]> {
    ...
    console.log(
      `[Timetable] Fetching for Class: ${classId}, Section: ${sectionId}`,
    );
    ...
    console.log(
      `[Timetable] Found ${results.length} entries for class ${classId}`,
    );
    return results;
  }
  ...
  async create(data: any): Promise<Timetable> {
    ...
    console.log(
      `[Timetable] Creating: ${dayOfWeek} ${startTime}-${endTime} | Class: ${classId} | Section: ${sectionId} | Subject: ${subjectId} | Teacher: ${teacherId}`,
    );
    ...
    if (sectionId && sectionId !== 'null') {
      sectionEntity = await this.sectionRepository.findOne({
        where: { id: sectionId } as any,
      });
      if (!sectionEntity)
        console.warn(
          `[Timetable] Section ${sectionId} not found, saving without section`,
        );
    }
    ...
    const teacherEntity =
      await this.usersService.ensureTeacherProfile(teacherId);
    if (!teacherEntity)
      throw new NotFoundException(
        `Teacher profile not found for user: ${teacherId}`,
      );
    ...
    console.log(
      `[Timetable] Saved entry ID: ${saved.id}, classId: ${(saved as any).classId}`,
    );
    return saved;
  }
